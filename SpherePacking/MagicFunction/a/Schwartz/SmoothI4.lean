...
def hf (t : ℝ) : ℂ := (-1 : ℂ) * (φ₀'' (arg t) * ((z₄' t : ℂ) - 1) ^ (2 : ℕ))
...
private lemma arg_eq_neg_one_div (t : ℝ) (ht : t ∈ Ioo (0 : ℝ) 1) :
    arg t = (-1 : ℂ) / ((-t : ℂ) + I) := by
  simpa [arg, sub_eq_add_neg, add_left_comm, add_comm, add_assoc] using
    congrArg (fun z : ℂ => (-1 : ℂ) / (z - 1)) (z₄'_eq_of_mem (t := t) (mem_Icc_of_Ioo ht))

private lemma arg_im_eq (t : ℝ) (ht : t ∈ Ioo (0 : ℝ) 1) :
    (arg t).im = 1 / (t ^ 2 + 1) := by
  simpa [arg_eq_neg_one_div (t := t) ht] using im_neg_one_div_neg_ofReal_add_I (t := t)
...
lemma continuousOn_hf : ContinuousOn hf (Ioo (0 : ℝ) 1) := by
  ...
  have harg : ContinuousOn arg (Ioo (0 : ℝ) 1) := by
    ...
    simpa [sub_eq_add_neg] using (continuous_z₄'.continuousOn.sub continuousOn_const)
  have hpow : ContinuousOn (fun t : ℝ => ((z₄' t : ℂ) - 1) ^ (2 : ℕ)) (Ioo (0 : ℝ) 1) := by
    simpa [sub_eq_add_neg] using
      (Continuous.continuousOn ((continuous_z₄'.sub continuous_const).pow 2))
